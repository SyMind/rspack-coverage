...
`,
    ...
    expect(manifest.modules.some((module) => module.identifier.includes("|"))).toBe(true);
    expect(manifest.modules.some((module) => module.identifier === "[unknown module]")).toBe(false);
    ...
    const referenceGraph = (await fetch(
      `${origin}/__rspack_coverage__/api/modules/${encodeURIComponent(moduleId)}/references?direction=both&depth=3`,
      { headers },
    ).then((response) => response.json())) as {
      graph: {
        nodes: Array<{ module: { id: string; resource: string | null } }>;
        edges: Array<{ originId: string; targetId: string }>;
      };
    };
    const moduleForFile = (filename: string) =>
      manifest.modules.find((module) => module.identifier.endsWith(`/${filename}`)) ??
      manifest.modules.find(
        (module) =>
          module.resource?.endsWith(`/${filename}`) ||
          module.identifier.split("|")[0]?.endsWith(`/${filename}`),
      );
    const indexModule = moduleForFile("index.js");
    const dependencyModule = moduleForFile("dependency.js");
    const leafModule = moduleForFile("leaf.js");
    expect(indexModule).toBeTruthy();
    expect(dependencyModule).toBeTruthy();
    expect(leafModule).toBeTruthy();
    expect(referenceGraph.graph.nodes.map(({ module }) => module.id)).toEqual(
      expect.arrayContaining([indexModule?.id, dependencyModule?.id, leafModule?.id]),
    );
    expect(referenceGraph.graph.edges).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          originId: indexModule?.id,
          targetId: dependencyModule?.id,
        }),
        expect.objectContaining({
          originId: dependencyModule?.id,
          targetId: leafModule?.id,
        }),
      ]),
    );
    expect(referenceGraph.graph.edges.some((edge) => edge.originId === edge.targetId)).toBe(false);
