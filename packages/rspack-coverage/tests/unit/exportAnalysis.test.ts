...
describe("export usage analysis", () => {
  ...
  it("attributes CommonJS require member uses to the export that carries them", () => {
    const result = parseExports(
      [
    ...
    expect(result.importUsages).toContainEqual({
      request: "./tex/normal.js",
      importedName: "normal",
      range: {
  ...
  it("combines stats and exact ModuleGraph references without losing an unused result", async () => {
    ...
    expect(actions?.moduleInstances[0]).toMatchObject({
      identifier: "/project/src/exports.ts",
      readableIdentifier: "./src/exports.ts",
      resource: "/project/src/exports.ts",
    });
