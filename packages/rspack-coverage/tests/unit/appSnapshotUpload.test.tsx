...
const sourceDrawer = vi.hoisted(() => vi.fn((_props: unknown) => null));

vi.mock("../../src/ui/lib/api.js", () => api);
vi.mock("../../src/ui/components/SourceDrawer.js", () => ({ SourceDrawer: sourceDrawer }));
...
describe("snapshot upload UI", () => {
  ...
  it("restores a module and export selection from a deep-link URL", async () => {
    const nextBuild = build("current-build-hash");
    nextBuild.modules = [
      {
        id: "target",
        identifier: "javascript/esm|/project/src/exports.ts",
        readableIdentifier: "./src/exports.ts",
        name: "./src/exports.ts",
    ...
    nextReport.files = [
      {
        id: "project/src/exports.ts",
        path: "project/src/exports.ts",
        displayPath: "src/exports.ts",
        category: "first-party",
        metrics: nextReport.metrics,
        moduleMetrics: nextReport.metrics,
        chunks: ["main"],
        loadedChunks: ["main"],
        moduleIds: ["target"],
        duplicated: false,
    ...
    api.loadBuild.mockResolvedValueOnce(nextBuild);
    api.loadCoverageAnalysisStatus.mockResolvedValueOnce({
      status: "complete",
      report: nextReport,
    });
    history.replaceState(null, "", "/?module=target&export=pdf");

    render(<App />);

    await waitFor(() => {
      const props = sourceDrawer.mock.calls.at(-1)?.[0];
      expect(props).toEqual(
        expect.objectContaining({
          file: nextReport.files[0],
          moduleId: "target",
          initialExportName: "pdf",
          restoreFromUrl: true,
          module: nextBuild.modules[0],
        }),
      );
    });
  });
