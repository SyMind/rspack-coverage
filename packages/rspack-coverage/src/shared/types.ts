...
export interface ModuleReferenceGraphNode {
  module: BuildModule;
  /** Signed distance from the selected module: importers are negative, dependencies positive. */
  depth: number;
}

export interface ModuleReferenceGraphEdge extends BuildReference {
  /** Number of captured dependency records represented by this module-to-module edge. */
  referenceCount: number;
}

export interface ModuleReferenceGraphResponse {
  direction: ReferenceDirection;
  requestedDepth: number;
  reachedDepth: number;
  nodes: ModuleReferenceGraphNode[];
  edges: ModuleReferenceGraphEdge[];
  truncated: boolean;
  truncation: {
    depth: boolean;
    nodes: boolean;
...
export interface ModuleReferencesResponse {
  ...
  /** Bounded, on-demand transitive topology for the Module Graph view. */
  graph?: ModuleReferenceGraphResponse;
}
...
export interface ExportImporterChainResponse {
  ...
  precision?: "native" | "mixed" | "source-inferred";
  diagnostics?: string[];
  truncated: boolean;
  maxDepth: number;
}
