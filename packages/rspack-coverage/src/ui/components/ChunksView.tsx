    ...
    <section className="panel data-panel">
      ...
      <div className="data-table chunk-table">
        ...
        {chunks.map((chunk) => {
          ...
          const filenames = chunk.files.join(", ") || "No JavaScript asset";
          return (
            <div className={`data-row ${chunk.loaded ? "" : "is-not-loaded"}`} key={chunk.id}>
              <span className="chunk-name">
                <strong {...copyablePathProps(chunkName)}>{chunkName}</strong>
                <small {...copyablePathProps(filenames)}>{filenames}</small>
              </span>
              <span>
