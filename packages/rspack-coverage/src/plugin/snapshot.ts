...
function collectModules(rawModules: any[], entryIdentifiers: Set<string>): BuildModule[] {
  ...
  const visit = (raw: any, inheritedChunks: string[], nested: boolean): void => {
    ...
    if (typeof raw?.identifier !== "string" || !raw.identifier) {
      for (const child of raw?.modules ?? []) visit(child, chunks, nested);
      return;
    }
...
function collectReferences(
  ...
  capture?: NativeExportUsageCapture,
): { unmapped: number } {
  if (capture?.referencesAvailable) {
    const rawLookup = rawModuleLookup(records);
    let activeRawOrigin: any | null = null;
    let activeCapturedOrigin: CapturedModuleSource | null = null;
    let unmapped = 0;
    for (const rawReference of capture.referenceEntries()) {
      const origin = buildModuleForIdentifier(
        rawReference.originIdentifier,
        rawReference.originLayer,
        lookup,
      );
      const target = buildModuleForIdentifier(
        rawReference.targetIdentifier,
        rawReference.targetLayer,
        lookup,
      );
      if (!origin || !target) {
        unmapped += 1;
      ...
      // A final ConcatenatedModule can make several logical modules resolve to
      // the same emitted container. Keep the pre-concatenation topology, but
      // never persist a container-induced self edge.
      if (origin.id === target.id) continue;
      const rawOrigin = rawModuleForIdentifier(
        rawReference.originIdentifier,
        rawReference.originLayer,
        rawLookup,
      );
      ...
      const traced = traceReferenceLocation(activeCapturedOrigin, rawReference.location);
      const identity = `${origin.id}\0${target.id}\0${rawReference.dependencyId}\0${
        rawReference.dependencyType ?? ""
      }\0${rawReference.request ?? ""}\0${JSON.stringify(rawReference.location)}\0${JSON.stringify(
        rawReference.exports,
      )}`;
      references.add({
        id: `ref_${shortHash(identity)}`,
        originId: origin.id,
        targetId: target.id,
        dependencyType: rawReference.dependencyType,
        request: rawReference.request,
        exports: rawReference.exports,
        active: rawReference.active,
        location: rawReference.location,
        sourcePath: traced?.sourcePath ?? origin.resource,
        sourceLocation: traced?.sourceLocation ?? rawReference.location,
      });
    }
    if (activeRawOrigin) originalSources.release(activeRawOrigin);
    references.finish();
...
function rawModuleLookup(records: RawModuleRecord[]): Map<string, RawModuleRecord[]> {
  const lookup = new Map<string, RawModuleRecord[]>();
  for (const record of records) {
    const identifier = moduleIdentifier(record.module);
    const candidates = lookup.get(identifier) ?? [];
    candidates.push(record);
    lookup.set(identifier, candidates);
  }
  return lookup;
}

function rawModuleForIdentifier(
  identifier: string,
  layer: string | null,
  lookup: Map<string, RawModuleRecord[]>,
): any | null {
  const candidates = lookup.get(identifier) ?? [];
...
export function createBuildSnapshot(
  ...
  try {
    ...
    const referenceResult = collectReferences(
      compilation,
      rawModuleRecords,
      modulesByIdentifier,
      originalSources,
    ...
    const discardedModuleReferences =
      (exportUsageCapture?.discardedReferences ?? 0) + referenceResult.unmapped;
    if (discardedModuleReferences > 0) {
      diagnostics.push({
        severity: "warning",
        message: `Rspack Coverage skipped ${discardedModuleReferences.toLocaleString()} malformed or unmapped pre-concatenation module reference(s).`,
      });
    }
